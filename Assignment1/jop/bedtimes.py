...
def plot_times(time_column):

    time_list = time_column.tolist()
    # Check for NaN values
    nan_values = pd.isna(time_list)
    # num_missing = nan_values.sum()

    # Convert nan to 'NaN'
    time_list = ['NaN' if isinstance(x, float) and np.isnan(x) else x for x in time_list]
